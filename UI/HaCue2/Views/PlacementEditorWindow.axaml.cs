...
public partial class PlacementEditorWindow : Window
{
    ...
    {
        InitializeComponent();
        Closing += (_, _) => (DataContext as InspectorViewModel)?.EndPlacementGesture();
    }

    private void InitializeComponent() => AvaloniaXamlLoader.Load(this);

    private void OnPlacementGesture(object? sender, PlacementGesture gesture) =>
        (DataContext as InspectorViewModel)?.ApplyPlacementGesture(gesture);

    private void OnPlacementGestureCompleted(object? sender, EventArgs e) =>
        (DataContext as InspectorViewModel)?.EndPlacementGesture();
}
