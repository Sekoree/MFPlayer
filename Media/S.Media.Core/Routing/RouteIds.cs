...
public readonly record struct InputId(Guid Value)
{
    ...
    public override string ToString() => $"Input({Value.ToString("N").AsSpan(0, 8)})";
}
...
public readonly record struct EndpointId(Guid Value)
{
    ...
    public override string ToString() => $"Endpoint({Value.ToString("N").AsSpan(0, 8)})";
}
...
public readonly record struct RouteId(Guid Value)
{
    ...
    public override string ToString() => $"Route({Value.ToString("N").AsSpan(0, 8)})";
}
