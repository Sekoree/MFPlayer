    ...
    IDisposable
{
    ...
    /// <summary>
    /// Device-side buffering in periods, requested at open and used by
    /// <see cref="SubmitToOutputLatency"/>. Six rather than miniaudio's default three, because this
    /// output's data callback is MANAGED code and the periods are the only buffer between a GC
    /// pause landing on the callback thread and an audible underrun.
    /// </summary>
    /// <remarks>
