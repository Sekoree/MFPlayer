...
public sealed class AVRouter : IAVRouter
{
    ...
    private volatile EndpointEntry[] _pushAudioEndpointsSnapshot = [];
    ...
    private bool _disposed => Volatile.Read(ref _disposedInt) != 0;
    ...
    private readonly Dictionary<EndpointId, double> _pushAudioFrameAccumulators = new();
    ...
    private readonly System.Diagnostics.Stopwatch _diagnosticsStopwatch = System.Diagnostics.Stopwatch.StartNew();
    ...
    public void UnregisterEndpoint(EndpointId id)
    {
        lock (_lock)
        {
            ...
            _pushAudioFrameAccumulators.Remove(id);
    ...
    private void RebuildEndpointsSnapshot()
    {
        ...
        var all = _endpoints.Values.ToArray();
        _endpointsSnapshot = all;
        _pushAudioEndpointsSnapshot = Array.FindAll(all, e => e.Audio is not null and not IPullAudioEndpoint);
    ...
    public void SetRouteGain(RouteId id, float gain)
    {
        if (!_routes.TryGetValue(id, out var route))
            throw new MediaRoutingException($"Route {id} is not registered.");
        Volatile.Write(ref route.Gain, gain);
    }
    ...
    public void SetEndpointGain(EndpointId id, float gain)
    {
        ...
        Volatile.Write(ref entry.Gain, gain);
    }
    ...
    private RouteId CreateVideoRoute(InputEntry inp, EndpointEntry ep, VideoRouteOptions options)
    {
        ...
        lock (_lock)
        {
            ...
            foreach (var r in _routes.Values)
            {
                if (r.Kind == InputKind.Video && r.InputId == inp.Id && r.EndpointId == ep.Id)
                {
                    existing = r;
                    break;
                }
    ...
    {
        if (_diagnosticsStopwatch.ElapsedMilliseconds < 200) return;
    ...
    private void PushAudioTick(double elapsedSeconds)
    {
        ...
        var pushAudioEndpoints = _pushAudioEndpointsSnapshot;
        foreach (var ep in pushAudioEndpoints)
        {
            ...
            foreach (var route in routes)
            {
                ...
                if (outFormat is null)
                {
                    ...
                    else
                    {
                        ...
                        _pushAudioFrameAccumulators.TryGetValue(ep.Id, out double accum);
            ...
            var destBuf = GetOrGrowScratch(_pushDestScratchBuffers, ep.Id, destSamples);
            ...
            foreach (var route in routes)
            {
                ...
                var scratch = GetOrGrowScratch(_scratchBuffers, ep.Id, srcSamples);
                ...
                if (needsResample)
                {
                    ...
                    if (route.ResampleScratch == null || route.ResampleScratch.Length < rsSamples)
                        route.ResampleScratch = new float[rsSamples];
                    var rsSpan = route.ResampleScratch.AsSpan(0, rsSamples);
                    rsSpan.Clear();
                ...
                if (route.BakedChannelMap is not null)
                {
                    ...
                    var mappedBuf = GetOrGrowScratch(_pushMappedScratchBuffers, ep.Id, mappedSamples);
    ...
    private void PushVideoTick()
    {
        ...
        foreach (var route in routes)
        {
            ...
            if (!bypassScheduledGate)
            {
                ...
                {
                    if (_pushVideoPending.TryRemove(route.Id, out var stale))
    ...
    private sealed class AudioFillCallbackForEndpoint : IAudioFillCallback
    {
        ...
        public void Fill(Span<float> dest, int frameCount, AudioFormat endpointFormat)
        {
            ...
            var routesByEp = _router._audioRoutesByEndpoint;
            if (!routesByEp.TryGetValue(_endpoint.Id, out var routes) || routes.Length == 0) return;
            for (int i = 0; i < routes.Length; i++)
            {
                ...
                if (route.Resampler is not null)
                {
                    ...
                    var outScratch = GetOrGrowScratch(_router._outputScratchBuffers, _endpoint.Id, outSamples);
                ...
                else if (route.BakedChannelMap is not null)
                {
                    ...
                    var outScratch = GetOrGrowScratch(_router._outputScratchBuffers, _endpoint.Id, mappedSamples);
    ...
    /// <summary>
    /// §3.14 / R6, §3.26 / P3, §8.4 — unified RT-safe accessor for per-endpoint
    /// scratch buffers. Fast path is a single <c>TryGetValue</c> + length check;
    /// the grow path uses <c>AddOrUpdate</c> with geometric growth so the RT
    /// thread does not allocate in steady state.
    /// </summary>
    private static float[] GetOrGrowScratch(ConcurrentDictionary<EndpointId, float[]> dict, EndpointId id, int minSize)
    {
        if (dict.TryGetValue(id, out var buf) && buf.Length >= minSize)
            return buf;
        return dict.AddOrUpdate(id,
            _ => new float[minSize],
            (_, old) => old.Length >= minSize ? old : new float[Math.Max(minSize, old.Length * 2)]);
    }
