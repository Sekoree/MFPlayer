...
internal sealed unsafe class FFmpegAudioChannel : IAudioChannel, ISeekableInput, IDecodableChannel
{
    ...
    private void OpenCodec()
    {
        ...
        int paramsRet = ffmpeg.avcodec_parameters_to_context(_codecCtx, _stream->codecpar);
        if (paramsRet < 0) throw new MediaOpenException($"avcodec_parameters_to_context failed: {paramsRet}");
        if (_threadCount >= 0)
    ...
    private void InitSwr()
    {
        ...
        if (ret < 0) throw new MediaOpenException($"swr_init failed: {ret}");
    }
    ...
    public void RaiseEndOfStream()
    {
        ...
        }, (this, handler), preferLocal: false);
    }
