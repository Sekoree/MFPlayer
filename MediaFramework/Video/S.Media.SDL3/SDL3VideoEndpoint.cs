    ...
    : IPullVideoEndpoint, IClockCapableEndpoint, IVideoColorMatrixReceiver, IVideoPresentationClockOverridable, IVideoEndpointInputFormatHint
{
    ...
    // Cached HUD output to avoid per-frame string allocations (A-1/A-2).
    private string[]? _cachedHudLines;
    private bool _hudLinesDirty = true;
    private string _cachedHudClockName = "n/a";
    private double _hudUpdateIntervalSeconds = 0.1;
    ...
    /// <summary>
    /// How often the HUD overlay recalculates FPS and rebuilds its text.
    /// Lower values give smoother FPS readouts but cost more string allocations.
    /// Default: 100 ms.
    /// </summary>
    public TimeSpan HudUpdateInterval
    {
        get => TimeSpan.FromSeconds(_hudUpdateIntervalSeconds);
        set => _hudUpdateIntervalSeconds = Math.Max(0.01, value.TotalSeconds);
    }
    ...
    /// <inheritdoc/>
    void IVideoEndpoint.ReceiveFrame(in VideoFrameHandle handle)
    {
    ...
    public void OverridePresentationClock(IMediaClock? clock)
    {
        ...
        _cachedHudClockName = clock is not null
            ? $"{clock.GetType().Name} (override)"
            : (_clock?.GetType().Name) ?? "n/a";
        _hudLinesDirty = true;
    ...
    public void Open(string title, int width, int height, VideoFormat format)
    {
        ...
        _cachedHudClockName = _presentationClockOverride is not null
            ? $"{_presentationClockOverride.GetType().Name} (override)"
            : _clock.GetType().Name;
    ...
    private void RenderLoop()
    {
        ...
        while (!token.IsCancellationRequested && !_closeRequested)
        {
            ...
            try
            {
                ...
                {
                    var vf = frame.Value;
                    if (!_hasUploadedFrame || _lastUploadedWidth != vf.Width || _lastUploadedHeight != vf.Height || _inputFormat.PixelFormat != vf.PixelFormat)
                    {
                        lock (_inputFormatLock)
                        {
                ...
                if (ShowHud)
                {
                    ...
                    _hudFrameCount++;
                    if (elapsed >= _hudUpdateIntervalSeconds)
                    {
                    ...
                    if (_hudLinesDirty || _cachedHudLines == null)
                    {
                        ...
                        _cachedHudLines = stats.ToLines();
                        _hudLinesDirty = false;
                    }
                    _renderer!.DrawHud(_cachedHudLines);
                }
