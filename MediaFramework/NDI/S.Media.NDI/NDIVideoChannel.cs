...
#pragma warning restore CS0067
    ...
    private void CaptureLoop()
    {
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                _hasLastPts = true;
                long ptsTicks = (long)(tsSecs * TimeSpan.TicksPerSecond);
    ...
    private void RaiseBufferUnderrun()
    {
        ...
        }, (this, handler, pos), preferLocal: false);
    }
    ...
    private static (YuvColorMatrix Matrix, YuvColorRange Range) ParseNdiColorMeta(string? xml)
    {
        ...
        ReadOnlySpan<char> scope = xml.AsSpan();
        int tagStart = xml.IndexOf("<ndi_color_space", StringComparison.OrdinalIgnoreCase);
        if (tagStart >= 0)
        {
            int tagEnd = xml.IndexOf('>', tagStart);
            if (tagEnd > tagStart)
                scope = xml.AsSpan(tagStart, tagEnd - tagStart + 1);
        }

        var colorspace = TryReadAttributeSpan(scope, "colorspace");
        var range = TryReadAttributeSpan(scope, "range");

        YuvColorMatrix matrix = colorspace.IsEmpty ? YuvColorMatrix.Auto
            : colorspace.Contains("2020", StringComparison.OrdinalIgnoreCase) ? YuvColorMatrix.Bt2020
            : colorspace.Contains("709", StringComparison.OrdinalIgnoreCase) ? YuvColorMatrix.Bt709
            : colorspace.Contains("601", StringComparison.OrdinalIgnoreCase) ? YuvColorMatrix.Bt601
            : YuvColorMatrix.Auto;

        YuvColorRange yr = range.IsEmpty ? YuvColorRange.Auto
            : range.Contains("Full", StringComparison.OrdinalIgnoreCase) ? YuvColorRange.Full
            : range.Contains("Limited", StringComparison.OrdinalIgnoreCase) ? YuvColorRange.Limited
            : YuvColorRange.Auto;

        return (matrix, yr);
    }

    private static ReadOnlySpan<char> TryReadAttributeSpan(ReadOnlySpan<char> scope, ReadOnlySpan<char> name)
    {
        ...
        while (idx >= 0)
        {
            ...
            if (eq < scope.Length && scope[eq] == '=')
            {
                ...
                if (eq < scope.Length && (scope[eq] == '"' || scope[eq] == '\''))
                {
                    char quote = scope[eq];
                    int valStart = eq + 1;
                    int valEnd = scope[valStart..].IndexOf(quote);
                    if (valEnd > 0)
                        return scope.Slice(valStart, valEnd);
                }
            }
            var remaining = scope[(idx + 1)..];
            int nextIdx = remaining.IndexOf(name, StringComparison.OrdinalIgnoreCase);
            idx = nextIdx >= 0 ? idx + 1 + nextIdx : -1;
        }
