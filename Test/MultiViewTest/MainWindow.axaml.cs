...
public partial class MainWindow : Window
{
    ...
    public MainWindow()
    {
        ...
        SummaryText.Text = "MultiViewTest — 4× AvaloniaVideoOutput + AvaloniaOpenGLHostControl";
    }
    ...
    private void InitializePlayback()
    {
        ...
        if (inputUri is null)
        {
            StatusText.Text = "No input found. Pass a file path as arg[0] or set SMEDIA_TEST_INPUT.";
            return;
        }
    ...
    {
        if (_videoSource is null || _outputs.Length == 0) return;
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            if (read != MediaResult.Success)
            {
                ...
                {
                    StatusText.Text = $"ReadFrame ended: code={read}, semantic={ErrorCodeRanges.ResolveSharedSemantic(read)}";
                });
                await Task.Delay(20, cancellationToken);
                continue;
            }
    ...
    private void RefreshStatus()
    {
        ...
        StatusText.Text = $"{state} | pos={pos:0.###}s / {durText}s | frame={_videoSource.CurrentFrameIndex} | 4 outputs | hud={(_hudEnabled ? "on" : "off")}";
    }
    ...
    private void DisposePlayback()
    {
        ...
        try { _playbackLoopTask?.Wait(TimeSpan.FromSeconds(1)); }
        catch { /* best-effort */ }

        if (_videoSource is not null) _ = _videoSource.Stop();
    ...
    {
        var argPath = Program.LaunchArgs.FirstOrDefault(a => !a.StartsWith("--"));
        if (string.IsNullOrWhiteSpace(argPath))
        {
            argPath = GetArg(Program.LaunchArgs, "--input");
        }

        if (!string.IsNullOrWhiteSpace(argPath) && File.Exists(argPath))
            return new Uri(Path.GetFullPath(argPath)).AbsoluteUri;

        var env = Environment.GetEnvironmentVariable("SMEDIA_TEST_INPUT");
        if (!string.IsNullOrWhiteSpace(env) && File.Exists(env))
            return new Uri(Path.GetFullPath(env)).AbsoluteUri;
